...
class TestMoveCollectionFolder:
    def test_moves_all_files(self, svc: FolderMoveService, tmp_path: Path) -> None:
        ...
        old_dir.mkdir()
        (old_dir / "FS22_ModA.zip").write_bytes(b"modA")
        (old_dir / "FS22_ModB.zip").write_bytes(b"modB")

        failed = svc.move_collection_folder(old_dir, new_dir)

        assert failed == []
        assert (new_dir / "FS22_ModA.zip").read_bytes() == b"modA"
        assert (new_dir / "FS22_ModB.zip").read_bytes() == b"modB"
        assert not (old_dir / "FS22_ModA.zip").exists()
    ...
    def test_continues_after_one_failure(self, svc: FolderMoveService, tmp_path: Path, monkeypatch) -> None:
        ...
        assert failed == ["FS22_ModA.zip"]
        assert (new_dir / "FS22_ModB.zip").exists()
...
class TestMoveSourceFolder:
    ...
    def test_relinks_symlink_to_new_collection(self, svc: FolderMoveService, tmp_path: Path) -> None:
        ...
        (old_collection / "FS22_ModA.zip").write_bytes(b"modA")
        os.symlink(old_collection / "FS22_ModA.zip", old_dir / "FS22_ModA.zip")

        failed = svc.move_source_folder(old_dir, new_dir, new_collection)

        assert failed == []
        link = new_dir / "FS22_ModA.zip"
        assert link.is_symlink()
        assert os.readlink(link) == str(new_collection / "FS22_ModA.zip")
        assert not (old_dir / "FS22_ModA.zip").exists()
