...
def populated(dirs):
    ...
    (collection / "FS22_ModA.zip").write_bytes(b"modA")
    (collection / "FS22_ModB.zip").write_bytes(b"modB")
    return source, collection
...
class TestActivate:
    def test_creates_symlinks(self, svc: LinkService, populated) -> None:
        ...
        svc.activate(["FS22_ModA.zip", "FS22_ModB.zip"], source, collection)
        assert (source / "FS22_ModA.zip").is_symlink()
        assert (source / "FS22_ModB.zip").is_symlink()

    def test_symlinks_point_to_collection(self, svc: LinkService, populated) -> None:
        source, collection = populated
        svc.activate(["FS22_ModA.zip"], source, collection)
...
class TestErrorHandling:
    ...
    def test_link_error_message_contains_filename(self, svc: LinkService, populated) -> None:
        ...
        with patch("os.symlink", side_effect=PermissionError(errno.EPERM, "x")):
            with pytest.raises(LinkError) as exc_info:
                svc.activate(["FS22_SpecialMod.zip"], source, collection)
        assert "FS22_SpecialMod.zip" in str(exc_info.value)
